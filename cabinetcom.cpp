...
CabinetCom::CabinetCom(QObject *parent) : QObject(parent)
{
    ...
    connect(&heartbeatTicker,&QTimer::timeout,this,&CabinetCom::processBSSStateMachine);//500ms
...
//    a= 10/a;
//    qDebug() << a;
    heartbeatCounter+=HEARTBEAT_TICKER_INTERVAL_mS;   // bien nay se bi reset boi signal readyread,moi khi nhan duoc ban tin thi readyread dc phat di.
...
void CabinetCom::requestDataSyncUpdate(){
    ...
    case SYNC_STATE_READ_CABINETS:
        ...
        if(getLoginFlag()){
            if(getSyncCabinetState() == SYNC_CABINET_STATE_SET_INIT)
                setSyncState(SYNC_STATE_READ_BSS_STATE);
            break;
        }
...
void CabinetCom::processSwapState()
{
    ...
    case SWAP_RECORD_ST_SELECT_EMPTY_CAB:
        ...
        //else writeCabinetDoorState(swapRecord->getOldCab(),DOOR_ST_OPEN);

        break;
    case SWAP_RECORD_ST_WAITING_OLD_BP:
        if(swapRecord->isTimeout()){ // timeout tong?
...
CONTINUE_FLAG CabinetCom::getContinueFlag()
{
    return continueFlag;
}
void CabinetCom::setContinueFlag(CONTINUE_FLAG value)
{
    continueFlag = value;
}
