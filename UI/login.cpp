...
void Login::rejectLogin(){
    ...
    SoundExecution::getSoundExecution()->play(WRONG_PASS);
    }
void Login::on_btnCancel_clicked() {
    this->cabCom->setLoginFlag(false);
    loginTimeoutTimer->stop();
    runProgressBar->stop();
    if(SoundExecution::getSoundExecution()->getStatePlayer() == STATE_PLAYER_PLAYING)
        SoundExecution::getSoundExecution()->getPlayer()->stop();
    close();
}
